...
export default function OrderConfirmation() {
  ...
  return (
    <div style={{ paddingTop: 88, minHeight: '100vh', display: 'flex', alignItems: 'center', justifyContent: 'center', background: 'radial-gradient(ellipse at 30% 50%, rgba(232,184,0,.05) 0%, transparent 60%), var(--d)' }}>
      <div style={{ maxWidth: 560, width: '100%', padding: 40, background: 'var(--p)', border: '1px solid var(--b)' }}>
        <div style={{ textAlign: 'center', marginBottom: 32 }}>
          ...
          <div style={{ fontFamily: '"Barlow Condensed", sans-serif', fontWeight: 900, fontStyle: 'italic', fontSize: 40 }}>ORDER PLACED!</div>
        </div>

        {loading ? (
          <div style={{ color: 'var(--t)', textAlign: 'center' }}>Loading order details...</div>
        ) : order ? (
          <div>
            <div style={{ borderTop: '1px solid var(--b)', borderBottom: '1px solid var(--b)', padding: '16px 0', marginBottom: 20 }}>
              ...
              <div style={{ display: 'flex', justifyContent: 'space-between' }}>
                ...
                <span style={{ fontFamily: '"Barlow Condensed", sans-serif', fontWeight: 900, fontSize: 24, color: 'var(--y)' }}>${parseFloat(order.total).toFixed(2)}</span>
              </div>
            </div>
            <div style={{ fontSize: 13, color: 'var(--t)', lineHeight: 1.8, marginBottom: 24 }}>
              🛡 Your custom wheel build is now queued. Expect a confirmation email shortly.<br />
              ⏱ Estimated build time: <strong style={{ color: 'var(--w)' }}>3–5 weeks</strong>.
            </div>
          </div>
        ) : (
          <div style={{ color: 'var(--t)', fontSize: 13, marginBottom: 24 }}>
            Your payment was received. A confirmation email will follow shortly.
          </div>
        )}
