...
export default function Contact() {
  ...
  const handleSend = async () => {
    ...
    try {
      ...
      // Fallback: load via script tag if import fails
      if (!window.emailjs) {
      ...
      setSendError('Failed to send. Please email us directly at service@whossteering.com');
    } finally {
      setSending(false);
    }
  };
    ...
    return (
      <div style={{ paddingTop: 88, minHeight: '100vh', display: 'flex', alignItems: 'center', justifyContent: 'center', background: 'radial-gradient(ellipse at 30% 50%, rgba(232,184,0,.05) 0%, transparent 60%), var(--d)' }}>
        <div style={{ maxWidth: 480, width: '100%', padding: 48, background: 'var(--p)', border: '1px solid var(--b)', margin: '0 20px', textAlign: 'center' }}>
          ...
          <div style={{ fontSize: 14, color: 'var(--t)', lineHeight: 1.8, marginBottom: 8 }}>
            Your message has been sent directly to
          </div>
          <div style={{ fontFamily: 'Orbitron, monospace', fontSize: 11, color: 'var(--y)', letterSpacing: 2, marginBottom: 20 }}>
            service@whossteering.com
          </div>
          <div style={{ fontSize: 13, color: 'var(--t)', lineHeight: 1.7, marginBottom: 28 }}>
            We typically respond within <strong style={{ color: 'var(--w)' }}>24 hours</strong>.
          </div>
      ...
      <div style={{ padding: '48px 40px', display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 60, maxWidth: 1100, margin: '0 auto' }}>
        ...
        <div>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
            ...
            {sendError && (
              <div style={{ padding: '10px 14px', background: 'rgba(204,51,0,.1)', border: '1px solid #CC3300', color: '#FF6644', fontSize: 13 }}>
                {sendError}
              </div>
            )}

            <button className="btn" style={{ clipPath: 'none', alignSelf: 'flex-start' }}
